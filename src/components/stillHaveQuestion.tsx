      ...
      <div className="w-3/4 mx-auto max-lg:w-4/5 max-md:w-full">
        <div className="text-center flex flex-col justify-center items-center gap-4">
          <h1 className="text-5xl max-md:text-4xl font-semibold text-blue-500">
            {stillHaveQuestionsSection.title}
          </h1>
          <p className="text-xl">{stillHaveQuestionsSection.brief}</p>
        </div>
        <div className="grid grid-cols-2 grid-rows-1 gap-5 max-md:grid-cols-1 max-md:grid-rows-2 text-center mt-10 ">
          {contacts.map((e: any, i: number) => {
            return (
              <div
                key={i}
                className="w-full rounded-md border-2 border-black/20 p-5 pt-28 max-sm:pt-20  flex items-end"
              >
                <div className="w-full">
                  <h2 className="text-2xl  font-semibold">
                    {e.fields.contact}
                  </h2>
                  <p className="text-xl text-gray-500">{e.fields.brief}</p>
                </div>
              </div>
            );
          })}
